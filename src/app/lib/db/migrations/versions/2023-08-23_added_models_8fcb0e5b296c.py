...
Revision ID: 8fcb0e5b296c
Revises: 
Create Date: 2023-08-23 15:10:50.524630
...
def schema_upgrades():
    ...
    op.create_table('cpe_business_product',
    ...
    sa.Column('created_at', sa.DateTimeUTC(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTimeUTC(timezone=True), nullable=False),
    sa.PrimaryKeyConstraint('id', name=op.f('pk_cpe_business_product')),
    sa.UniqueConstraint('key', name=op.f('uq_cpe_business_product_key')),
    sa.UniqueConstraint('name', name=op.f('uq_cpe_business_product_name'))
    )
    op.create_table('tag',
